                ...
                <div class="InfoWrap">
                    ...
                    <div class="choose">
                        <div class="chooseArea">
                            ...
                            <dl v-for="e in spuSaleAttrList" :key="e.id">
                                <dt class="title">{{e.saleAttrName}}</dt>
                                <dd
                                        changepirce="0" :class="{active:v.isChecked === '1'}"
                                        v-for="v in e.spuSaleAttrValueList" :key="v.id"
                                        @click="changeChecked(v,e.spuSaleAttrValueList)"
                        ...
                        <div class="cartWrap">
                            ...
                            <div class="add">
                                <a href="javascript:" @click="addGoodsToCart">加入购物车</a>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            changeChecked(v, spuSaleAttrValueList) {
                spuSaleAttrValueList.forEach(e => e.isChecked = "0")
                v.isChecked = "1"
            },
            async addGoodsToCart() {
                //dispatch  分发触发的意思，其实就是调用响应的actions函数
                //this.$store.dispatch('addOrUpdateCart',{skuId,skuNum}) 就是函数调用表达式，结果就是
                // actions当中异步函数的返回值，异步函数返回值一定是promise
                const {skuId, skuNum} = this
                try {
                    await this.$store.dispatch("addOrUpdataDoodsToCart", {skuId, skuNum})
                    alert("添加购物车成功")
                    // 成功需要跳转  编程式导航跳转
                    ...
                    sessionStorage.setItem("GOODSINFO_KEY", JSON.stringify(this.skuInfo))
